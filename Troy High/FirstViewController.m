...
- (void)viewDidLoad
{
    ...
    refreshControl.tintColor = [UIColor colorWithRed:0.7 green:0.0 blue:0.0 alpha:1.0];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath //Calls the action when you press on a cell in the table
{
	...
	long storyIndex = [indexPath indexAtPosition: [indexPath length] - 1]; //Creates a storyIndex variable to hold the current cell index
...
{
    NSDictionary *dictionary = [NSDictionary dictionaryWithObjectsAndKeys:@"Your user agent", @"User-Agent", nil];
    ...
    if ([[segue identifier] isEqualToString:@"AnnouncementView"]) //the identifier is used so that it links to the correct storyboard
    {
        ...
        NSLog(@"Sent to AV with link %@", storyLink);
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath //Basically just returns and popululates all of the cells in the application
{
	...
	long storyIndex = [indexPath indexAtPosition: indexPath.length -1]; //Index of the cell you are at
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    {
        NSRange match;
        match = [currentTitle rangeOfString: @">"];
        ...
        else
        {
            ...
            tempLink = [currentTitle substringWithRange: NSMakeRange(8, match.location-8)];
            
            [currentTitle replaceCharactersInRange: NSMakeRange(0, match.location+1) withString: @""];
            [currentTitle deleteCharactersInRange: [currentTitle rangeOfString: @"</a>"]];
            
            currentLink = [NSMutableString stringWithString: tempLink];
            
            currentLink = [[currentLink stringByReplacingOccurrencesOfString:@"\"" withString:@""] mutableCopy];
            
            currentLink = [[currentLink stringByReplacingOccurrencesOfString:@"#zoom=100" withString:@""] mutableCopy];
            
            NSLog(@"New Title: %@", currentTitle);
            NSLog(@"New Link : %@", currentLink);
        }
...
{
	NSLog(@"stories array has %lu items", (unsigned long)[stories count]); //Outputs the number of elements in the Array (will always be 10 because of Troy's website)
