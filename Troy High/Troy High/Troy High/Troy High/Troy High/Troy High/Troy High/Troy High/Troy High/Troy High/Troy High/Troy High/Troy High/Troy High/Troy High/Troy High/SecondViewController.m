...
- (void)viewDidLoad
{
    ...
    cellData = [[NSMutableArray alloc] initWithObjects:@"Bell Schedules", @"Campus Map", @"Food Menu", @"Attendance", @"ESLRs", @"Principal's Message", @"Vision Statement", nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath //Checks which cell was selected and appropriately directs the user to the appropriate vew with the information that they want 
{
    ...
    if(indexPath.row == 2)
    {
        [self performSegueWithIdentifier:@"Food" sender:nil];
        [tableView deselectRowAtIndexPath:[tableView indexPathForSelectedRow] animated:YES];
    }
